...
| 经验 | 从原卡提炼后局部细化的 13 条 active 经验 |
| 阅读目录 | 13 条经验、19 个主题；每层只披露直接子项 |
| Memory revision | `410b563c37b78acf42584c1a0179e96bd8918762ce4b696c401bb026f142fc05` |
| 数据格式 | `migloop-memory/1`、精简的 `migloop-case/2`；维护脚本兼容旧卡 `/1` |
| 阅读包格式 / 发布器 | `migloop-memory-files/1` / `0.7.2` |

本分支 `dev/rebuild-from-cards-20260921` 保存一次**从原卡、空库出发**的独立重建，不覆盖 `main` 上的旧版经验。模型为 GPT-5.6 Sol high，零上下文，只提供新版归并 skill、15 张卡片和输出位置；没有读取旧 27 条 lesson。提案与说明见 [本轮归并记录](proposals/2026-09-21-from-cards/notes.md)。

2026-09-22 在这 12 条上做了局部修订：Material 尺寸与单侧 inset 分开，成为 13 条；保留公开 API 与小例子，检查改成按需执行的可选项，不增加默认测试任务。这是 Codex 的维护修订，不是重跑上面的独立模型实验。详见 [局部细化记录](proposals/2026-09-22-lightweight-refinement/notes.md)。

本轮保留卡片的全部历史事实、具体修法和证据；lesson 提炼有适用条件的机制、动作与检查方法，不把项目变量、固定设备尺寸或字体比例作为新任务默认要求。43 个来源引用可解析，覆盖全部 15 张卡片。`active` 是本轮模型的维护判断，不是机械认证技术结论，也不表示已验证跨项目迁移收益。

新库最初的三份快照来自空库初始化、入卡、归并，随后增加本轮局部细化快照；它是新的 memory lineage，不是 main 旧 HEAD 的增量改写。旧快照和卡片仍保留，旧阅读目录可从 `main` 或提交 `32ddc87` 获取。当前阅读目录从新快照 `export --link-cards` 生成，来源链接指向仓库内确切卡片，不单独手改阅读正文。
...
当时的格式迁移同步更新了 3 个快照中的卡片哈希、73 处经验来源绑定及阅读目录链接。原始大卡和旧快照仍可在 Git 提交 `486b377` 中恢复；历史归并提案保留当时的旧 revision，未伪装成新提案。后续维护本分支时使用上表当前 revision。
...
store/
  ...
  snapshots/             保留旧库与本轮重建的不可变快照
  cases/<id>/<hash>.json  正文、证据引用、必要来源及简短核验状态
memory/
  index.md               分层阅读入口
  ui/                    组件、布局、渲染、文本、状态
  implementation/        ArkUI 实现约束
  manifest.json          阅读包文件校验和及来源绑定
proposals/
  2026-09-19-jetsnack915-v1/
    plan.yaml            原始归并提案
    notes.md             原始归并理由、未决项和审核记录
  2026-09-21-from-cards/
    plan.yaml            从原卡独立提炼的归并提案
    notes.md             本轮合并与拆分理由
    validation.md        原始运行及仓库阅读包的校验说明
  2026-09-22-lightweight-refinement/
    plan.json            局部修订提案，明确保留来源绑定
    notes.md             修改理由、版本与机械校验结果
```
