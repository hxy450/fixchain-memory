...
| 经验 | 从原卡重新提炼的 12 条 active 经验 |
| 阅读目录 | 12 条经验、19 个主题；每层只披露直接子项 |
| Memory revision | `3c8d212f557f1ef86369c96f5eed1d4074909db9722671cf5489f3e10e529a41` |
| 数据格式 | `migloop-memory/1`、精简的 `migloop-case/2`；维护脚本兼容旧卡 `/1` |
| 阅读包格式 / 发布器 | `migloop-memory-files/1` / `0.7.1` |
...
新库的三份快照来自空库初始化、入卡、归并，因此它是新的 memory lineage，不是旧 HEAD 的增量改写。旧快照和卡片仍保留，旧阅读目录可从 `main` 或提交 `32ddc87` 获取。当前阅读目录通过同一发布器重新 `export --link-cards`，让 GitHub 上的来源链接指向仓库内确切卡片；不修改 lesson 正文。
